...
use App\Model\Admin\ActivityGallery;
use App\Model\Admin\Activiy;
...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        $data['activites'] = Activiy::query()->with([ 'galleries' => function ($q) {
            $q->select(['id', 'activity_id', 'sort'])
                ->with(['image'])
                ->orderBy('sort', 'ASC');
        }])->latest()->get();
        $data['allImageGallery'] = ActivityGallery::query()->with('activity', 'image')->latest()->get();
    ...
    public function getListRooms() {
        ...
        $data['banner'] = BannerPage::query()->with('image')->where('id',2)->first();
    ...
    public function services(Request $request) {
       $services = Service::query()->with(['image', 'galleries.image'])
           ->where('status', 1)
           ->latest()->get();
       $banner = BannerPage::query()->with('image')->where('id', 3)->first();
    ...
    public function galleries(Request $request) {
        $data['banner'] = BannerPage::query()->with('image')
            ->where('id', 4)->first();
        $data['activites'] = Activiy::query()->with([ 'galleries' => function ($q) {
            $q->select(['id', 'activity_id', 'sort'])
                ->with(['image'])
        ...
        return view('site.galleries',$data);
    }
